...
//for only adding 
    ...
    getTitle(){
        if(this.state.roomId == -1){
            return <h2 style={{color:"lightsalmon"}} className="text-center">Add Room </h2>
        }
        else{
            return <h2 style={{color:"lightsalmon"}} className="text-center">Update Room</h2>
        }
    }
    ...
    render() {
        return (
            <div style={{backgroundImage:"url(https://www.itl.cat/pngfile/big/203-2039706_photo-wallpaper-texture-design-papers-material-illustration.jpg)",height:700,backgroundSize:1400}}>
                ...
                <button type="submit" class="btn btn-primary" onClick={this.editRoom} style={{backgroundColor:"lightsalmon"}}>Save</button>
